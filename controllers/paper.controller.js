...
const createPaper = async (req ,res,next ) => {
    try {
        ...
        const questionPaper = await paperModel.create({
            questions :  [...easyQuestionsStore , ...mediumQuestionsStore , ...hardQuestionsStore],
            title:"Test Paper" ,
            marks : marks 
        })
...
const makePaperTopicWise = async (req ,res ,next ) =>  { 
    try {   
        const { marks , easy  , medium , hard , topics } = req.body;
        if(!marks  || easy < 0 || medium < 0 || hard  < 0  || !topics ) { 
